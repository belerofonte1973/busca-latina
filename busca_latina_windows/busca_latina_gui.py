#!/usr/bin/env python3
"""
busca_latina_gui.py — Interface gráfica do Busca Latina
Windows 11 ARM / Snapdragon X — usa PyQt6 (wheels nativos ARM64)
"""
...
# ── ficheiro de configuração (Windows: %APPDATA%\BuscaLatina) ─────────────────
...
# ── fonte monoespaçada preferida no Windows ───────────────────────────────────
...
class BuscaLatina(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.entry.setPlaceholderText(
            "amor=exato · amor-=prefixo · -que=sufixo · -amor-=infixo · regex: usar . * + [ \\"
        )
        ...
        # ── splitter: lista de obras | resultados ─────────────────────────────
        ...
        self.btn_parar_ia.setFixedWidth(60)
        ...
        self.btn_gemini_chave.setFixedWidth(50)
        ...
        self.btn_ls.setToolTip("Lewis & Short — latim → inglês (offline, requer Diogenes)")
        self.btn_ls.clicked.connect(lambda: self._on_dicionario("ls"))
        ctrl_ia.addWidget(self.btn_ls)

        self.btn_lsj = QPushButton("LSJ")
        self.btn_lsj.setToolTip("Liddell-Scott-Jones — grego → inglês (offline, requer Diogenes)")
        ...
        self.btn_pronunciar.setToolTip("Pronuncia o texto seleccionado (edge-tts online)")
        ...
        self.btn_parar_som.setFixedWidth(60)
        ...
        self.btn_ipa.setToolTip("Transcrição IPA (requer espeak-ng instalado)")
        ...
        self.trans_out.setPlaceholderText(
            ...
            "L&S / LSJ     — dicionário offline (requer Diogenes)\n\n"
    ...
    def _on_search(self):
        ...
        self.thread = SearchThread(
            ...
            do_ll, do_perseus, self.spin_max.value(),
        )
    ...
    def _on_finished(self, total):
        ...
        if total == 0:
            self.text.setPlainText(
                f'Nenhuma ocorrência encontrada para "{self.entry.text()}".'
            )
    ...
    def _on_gemini_traduzir(self):
        ...
        if not chave:
            QMessageBox.warning(self, "Chave API em falta",
                                "Configure a chave Gemini clicando em «Chave» ao lado.\n"
                                "Obtenha gratuitamente em aistudio.google.com")
            return
    ...
    def _on_modelo_falhou(self, _nome: str):
        self.status_bar.showMessage(
            "Ollama não responde — abra o Ollama e execute: ollama serve"
        )
        self.btn_ollama.setEnabled(True)
        self.btn_comentario.setEnabled(True)
    ...
    def _rodar_traducao(self, texto: str, modo: str):
        if not texto.strip():
            return
        self.trans_out.setPlaceholderText("")
        self.trans_out.setText("A processar…")
    ...
    def _on_dicionario(self, modo: str):
        cursor   = self.text.textCursor()
        palavra  = cursor.selectedText().strip().split()[0] if cursor.selectedText().strip() else ""
        if not palavra:
...
def main():
    # Necessário no Windows para ícones e DPI correcto
    if sys.platform == "win32":
        try:
